...
# Versión modificada de install_dependencies para corregir el bug
# en la descarga de MaxMindDB GeoLite2-City, necesaria para el dashboard.
# Realizado por Enrique Rossel - KMHCORP - 11/3/2020.
...
if [ -e "config/config.cfg" ]; then
    ...
    sed -i "s|pathMaxMindDB=./data/GeoLite2-City/GeoLite2-City.mmdb|pathMaxMindDB=/var/www/misp-dashboard/data/GeoLite2-City/GeoLite2-City.mmdb|" config/config.cfg
    sed -i "s|path_countrycode_to_coord_JSON=./data/country_code_lat_long.json|path_countrycode_to_coord_JSON=/var/www/misp-dashboard/data/country_code_lat_long.json|" config/config.cfg
fi
